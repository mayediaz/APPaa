...
class festivos
{
	...
	public function esNoLaboral($dia,$mes)
	{
		//echo (int)$mes;
		if($dia=='' or $mes=='')
		{
		...
		if (isset($this->festivos[$this->ano][(int)$mes][(int)$dia]) || isset($this->dnl[date("w",strtotime("$dia-$mes-".$this->ano))]) || (isset($this->dlab[date("w",strtotime("$dia-$mes-".$this->ano))]) && ((date('H') >= '00' && date('H') < '07') || (date('H') >= '17' && date('H') < '23'))))
		{
			return true;
		}
		else
